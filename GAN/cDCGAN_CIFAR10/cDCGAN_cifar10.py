...
class generator(nn.Module):
    ...
    def __init__(self, d=128):
        ...
        self.deconv4 = nn.ConvTranspose2d(d*2, d, 1, 1, 0)
...
class discriminator(nn.Module):
    ...
    def __init__(self, d=128):
        ...
        self.conv4 = nn.Conv2d(d*4, d*8, 1, 1, 0)
...
def show_result(num_epoch, show = False, save = False, path = 'result.png'):
    ...
    for k in range(10*10):
        ...
        j = k % 10
        ax[i, j].cla()
        tmp = np.transpose(test_images[k].cpu().data.numpy(), (1,2,0))
        ax[i, j].imshow((tmp - np.min(tmp))/(np.max(tmp) - np.min(tmp)))
